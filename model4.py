...
class HyperNet(BaseClass):
    ...
    def all_att(self, ei_idx):
        all_att = torch.zeros(ei_idx.shape[0], 300).to(self.dataset.device)
        for i in range(ei_idx.shape[0]):
            ei = ei_idx[i]
            for edge in self.dataset.inc[ei.item()]:
                r = edge[0]
                for j in edge[1:]:

                    eij = torch.mm(torch.cat((self.E(ei).view(-1, 200), self.E(torch.tensor(j).to(self.dataset.device)).view(-1, 200), self.R(torch.tensor(r).to(self.dataset.device)).view(-1, 200)), dim=1), self.W0).view(300)
                    all_att[i] += eij
    ...
    def forward(self, batch, ms, bs):
        ...
        if batch.shape[1] == 3:
            ...
            e11_m = -self.leakyrelu(torch.mm(e11, self.a))
            e12_m = -self.leakyrelu(torch.mm(e12, self.a))
            e21_m = -self.leakyrelu(torch.mm(e21, self.a))
            e22_m = -self.leakyrelu(torch.mm(e22, self.a))


            e1_all = self.all_att(batch[:, 1])
            e2_all = self.all_att(batch[:, 2])

            e1_e2_att = torch.exp(e11_m) / torch.exp(-self.leakyrelu(torch.mm(e1_all, self.a)))
            e1_e1_att = torch.exp(e12_m) / torch.exp(-self.leakyrelu(torch.mm(e1_all, self.a)))
            e2_e1_att = torch.exp(e21_m) / torch.exp(-self.leakyrelu(torch.mm(e2_all, self.a)))
            e2_e2_att = torch.exp(e22_m) / torch.exp(-self.leakyrelu(torch.mm(e2_all, self.a)))
            ...
            e11_m = -self.leakyrelu(torch.mm(e11, self.a))
            e12_m = -self.leakyrelu(torch.mm(e12, self.a))
            e13_m = -self.leakyrelu(torch.mm(e13, self.a))
            e21_m = -self.leakyrelu(torch.mm(e21, self.a))
            e22_m = -self.leakyrelu(torch.mm(e22, self.a))
            e23_m = -self.leakyrelu(torch.mm(e23, self.a))
            e31_m = -self.leakyrelu(torch.mm(e31, self.a))
            e32_m = -self.leakyrelu(torch.mm(e32, self.a))
            e33_m = -self.leakyrelu(torch.mm(e33, self.a))

            e1_e1_att = torch.exp(e11_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m))
            e1_e2_att = torch.exp(e12_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m))
            e1_e3_att = torch.exp(e13_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m))
            e2_e1_att = torch.exp(e21_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m))
            e2_e2_att = torch.exp(e22_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m))
            e2_e3_att = torch.exp(e23_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m))
            e3_e1_att = torch.exp(e31_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m))
            e3_e2_att = torch.exp(e32_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m))
            e3_e3_att = torch.exp(e33_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m))
            ...
            e12_m = -self.leakyrelu(torch.mm(e12, self.a))
            e13_m = -self.leakyrelu(torch.mm(e13, self.a))
            e14_m = -self.leakyrelu(torch.mm(e14, self.a))
            e21_m = -self.leakyrelu(torch.mm(e21, self.a))
            e22_m = -self.leakyrelu(torch.mm(e22, self.a))
            e23_m = -self.leakyrelu(torch.mm(e23, self.a))
            e24_m = -self.leakyrelu(torch.mm(e24, self.a))
            e31_m = -self.leakyrelu(torch.mm(e31, self.a))
            e32_m = -self.leakyrelu(torch.mm(e32, self.a))
            e33_m = -self.leakyrelu(torch.mm(e33, self.a))
            e34_m = -self.leakyrelu(torch.mm(e34, self.a))
            e41_m = -self.leakyrelu(torch.mm(e41, self.a))
            e42_m = -self.leakyrelu(torch.mm(e42, self.a))
            e43_m = -self.leakyrelu(torch.mm(e43, self.a))
            e44_m = -self.leakyrelu(torch.mm(e44, self.a))

            e1_e1_att = torch.exp(e11_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m))
            e1_e2_att = torch.exp(e12_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m))
            e1_e3_att = torch.exp(e13_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m))
            e1_e4_att = torch.exp(e14_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m))
            e2_e1_att = torch.exp(e21_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m))
            e2_e2_att = torch.exp(e22_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m))
            e2_e3_att = torch.exp(e23_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m))
            e2_e4_att = torch.exp(e24_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m))
            e3_e1_att = torch.exp(e31_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m))
            e3_e2_att = torch.exp(e32_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m))
            e3_e3_att = torch.exp(e33_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m))
            e3_e4_att = torch.exp(e34_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m))
            e4_e1_att = torch.exp(e41_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m))
            e4_e2_att = torch.exp(e42_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m))
            e4_e3_att = torch.exp(e43_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m))
            e4_e4_att = torch.exp(e44_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m))
            ...
            e13_m = -self.leakyrelu(torch.mm(e13, self.a))
            e14_m = -self.leakyrelu(torch.mm(e14, self.a))
            e15_m = -self.leakyrelu(torch.mm(e15, self.a))
            e21_m = -self.leakyrelu(torch.mm(e21, self.a))
            e22_m = -self.leakyrelu(torch.mm(e22, self.a))
            e23_m = -self.leakyrelu(torch.mm(e23, self.a))
            e24_m = -self.leakyrelu(torch.mm(e24, self.a))
            e25_m = -self.leakyrelu(torch.mm(e25, self.a))
            e31_m = -self.leakyrelu(torch.mm(e31, self.a))
            e32_m = -self.leakyrelu(torch.mm(e32, self.a))
            e33_m = -self.leakyrelu(torch.mm(e33, self.a))
            e34_m = -self.leakyrelu(torch.mm(e34, self.a))
            e35_m = -self.leakyrelu(torch.mm(e35, self.a))
            e41_m = -self.leakyrelu(torch.mm(e41, self.a))
            e42_m = -self.leakyrelu(torch.mm(e42, self.a))
            e43_m = -self.leakyrelu(torch.mm(e43, self.a))
            e44_m = -self.leakyrelu(torch.mm(e44, self.a))
            e45_m = -self.leakyrelu(torch.mm(e45, self.a))
            e51_m = -self.leakyrelu(torch.mm(e51, self.a))
            e52_m = -self.leakyrelu(torch.mm(e52, self.a))
            e53_m = -self.leakyrelu(torch.mm(e53, self.a))
            e54_m = -self.leakyrelu(torch.mm(e54, self.a))
            e55_m = -self.leakyrelu(torch.mm(e55, self.a))


            e1_e1_att = torch.exp(e11_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m))
            e1_e2_att = torch.exp(e12_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m))
            e1_e3_att = torch.exp(e13_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m))
            e1_e4_att = torch.exp(e14_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m))
            e1_e5_att = torch.exp(e15_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m))
            e2_e1_att = torch.exp(e21_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m))
            e2_e2_att = torch.exp(e22_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m))
            e2_e3_att = torch.exp(e23_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m))
            e2_e4_att = torch.exp(e24_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m))
            e2_e5_att = torch.exp(e25_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m))
            e3_e1_att = torch.exp(e31_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m))
            e3_e2_att = torch.exp(e32_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m))
            e3_e3_att = torch.exp(e33_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m))
            e3_e4_att = torch.exp(e34_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m))
            e3_e5_att = torch.exp(e35_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m))
            e4_e1_att = torch.exp(e41_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m))
            e4_e2_att = torch.exp(e42_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m))
            e4_e3_att = torch.exp(e43_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m))
            e4_e4_att = torch.exp(e44_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m))
            e4_e5_att = torch.exp(e45_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m))
            e5_e1_att = torch.exp(e51_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m))
            e5_e2_att = torch.exp(e52_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m))
            e5_e3_att = torch.exp(e53_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m))
            e5_e4_att = torch.exp(e54_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m))
            e5_e5_att = torch.exp(e55_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m))
            ...
            e14_m = -self.leakyrelu(torch.mm(e14, self.a))
            e15_m = -self.leakyrelu(torch.mm(e15, self.a))
            e16_m = -self.leakyrelu(torch.mm(e16, self.a))
            e21_m = -self.leakyrelu(torch.mm(e21, self.a))
            e22_m = -self.leakyrelu(torch.mm(e22, self.a))
            e23_m = -self.leakyrelu(torch.mm(e23, self.a))
            e24_m = -self.leakyrelu(torch.mm(e24, self.a))
            e25_m = -self.leakyrelu(torch.mm(e25, self.a))
            e26_m = -self.leakyrelu(torch.mm(e26, self.a))
            e31_m = -self.leakyrelu(torch.mm(e31, self.a))
            e32_m = -self.leakyrelu(torch.mm(e32, self.a))
            e33_m = -self.leakyrelu(torch.mm(e33, self.a))
            e34_m = -self.leakyrelu(torch.mm(e34, self.a))
            e35_m = -self.leakyrelu(torch.mm(e35, self.a))
            e36_m = -self.leakyrelu(torch.mm(e36, self.a))
            e41_m = -self.leakyrelu(torch.mm(e41, self.a))
            e42_m = -self.leakyrelu(torch.mm(e42, self.a))
            e43_m = -self.leakyrelu(torch.mm(e43, self.a))
            e44_m = -self.leakyrelu(torch.mm(e44, self.a))
            e45_m = -self.leakyrelu(torch.mm(e45, self.a))
            e46_m = -self.leakyrelu(torch.mm(e46, self.a))
            e51_m = -self.leakyrelu(torch.mm(e51, self.a))
            e52_m = -self.leakyrelu(torch.mm(e52, self.a))
            e53_m = -self.leakyrelu(torch.mm(e53, self.a))
            e54_m = -self.leakyrelu(torch.mm(e54, self.a))
            e55_m = -self.leakyrelu(torch.mm(e55, self.a))
            e56_m = -self.leakyrelu(torch.mm(e56, self.a))
            e61_m = -self.leakyrelu(torch.mm(e61, self.a))
            e62_m = -self.leakyrelu(torch.mm(e62, self.a))
            e63_m = -self.leakyrelu(torch.mm(e63, self.a))
            e64_m = -self.leakyrelu(torch.mm(e64, self.a))
            e65_m = -self.leakyrelu(torch.mm(e65, self.a))
            e66_m = -self.leakyrelu(torch.mm(e66, self.a))

            e1_e1_att = torch.exp(e11_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e1_e2_att = torch.exp(e12_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e1_e3_att = torch.exp(e13_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e1_e4_att = torch.exp(e14_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e1_e5_att = torch.exp(e15_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e1_e6_att = torch.exp(e16_m) / (torch.exp(e11_m) + torch.exp(e12_m) + torch.exp(e13_m) + torch.exp(e14_m) + torch.exp(e15_m) + torch.exp(e16_m))
            e2_e1_att = torch.exp(e21_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e2_e2_att = torch.exp(e22_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e2_e3_att = torch.exp(e23_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e2_e4_att = torch.exp(e24_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e2_e5_att = torch.exp(e25_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e2_e6_att = torch.exp(e26_m) / (torch.exp(e21_m) + torch.exp(e22_m) + torch.exp(e23_m) + torch.exp(e24_m) + torch.exp(e25_m) + torch.exp(e26_m))
            e3_e1_att = torch.exp(e31_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e3_e2_att = torch.exp(e32_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e3_e3_att = torch.exp(e33_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e3_e4_att = torch.exp(e34_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e3_e5_att = torch.exp(e35_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e3_e6_att = torch.exp(e36_m) / (torch.exp(e31_m) + torch.exp(e32_m) + torch.exp(e33_m) + torch.exp(e34_m) + torch.exp(e35_m) + torch.exp(e36_m))
            e4_e1_att = torch.exp(e41_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e4_e2_att = torch.exp(e42_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e4_e3_att = torch.exp(e43_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e4_e4_att = torch.exp(e44_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e4_e5_att = torch.exp(e45_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e4_e6_att = torch.exp(e46_m) / (torch.exp(e41_m) + torch.exp(e42_m) + torch.exp(e43_m) + torch.exp(e44_m) + torch.exp(e45_m) + torch.exp(e46_m))
            e5_e1_att = torch.exp(e51_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e5_e2_att = torch.exp(e52_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e5_e3_att = torch.exp(e53_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e5_e4_att = torch.exp(e54_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e5_e5_att = torch.exp(e55_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e5_e6_att = torch.exp(e56_m) / (torch.exp(e51_m) + torch.exp(e52_m) + torch.exp(e53_m) + torch.exp(e54_m) + torch.exp(e55_m) + torch.exp(e56_m))
            e6_e1_att = torch.exp(e61_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
            e6_e2_att = torch.exp(e62_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
            e6_e3_att = torch.exp(e63_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
            e6_e4_att = torch.exp(e64_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
            e6_e5_att = torch.exp(e65_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
            e6_e6_att = torch.exp(e66_m) / (torch.exp(e61_m) + torch.exp(e62_m) + torch.exp(e63_m) + torch.exp(e64_m) + torch.exp(e65_m) + torch.exp(e66_m))
